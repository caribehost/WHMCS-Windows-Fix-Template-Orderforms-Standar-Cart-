...
jQuery(document).ready(function(){
    ...
    jQuery('#frmProductDomain').submit(function (e) {
        ...
        if (domainoption == 'register') {
            ...
            lookup.done(function (data) {
                jQuery.each(JSON.parse(data).result, function(index, domain) {
            ...
            spotlight.done(function(data) {
                if (typeof JSON.parse(data) != 'object' || JSON.parse(data).result.length == 0 || JSON.parse(data).result.error) {
            ...
            transfer.done(function (data) {
                if (typeof JSON.parse(data) != 'object' || JSON.parse(data).result.length == 0) {
                    jQuery('.domain-lookup-primary-loader').hide();
                    return;
                }
                jQuery.each(JSON.parse(data).result, function(index, domain) {
            ...
            otherDomain.done(function(data) {
                ...
                jQuery.each(JSON.parse(data).result, function(index, result) {
    ...
    jQuery('.btn-add-to-cart').on('click', function() {
        ...
        var addToCart = jQuery.post(
            ...
            if (JSON.parse(data).result == 'added') {
                ...
                if (resultDomain.length && !resultDomain.val()) {
                    ...
                    resultDomainPricing.val(JSON.parse(data).period).attr('name', 'domainsregperiod[' + domain +']');
                    if (btnDomainContinue.length > 0 && btnDomainContinue.is(':disabled')) {
                        btnDomainContinue.removeAttr('disabled');
                    }
                }
                jQuery('#cartItemCount').html(JSON.parse(data).cartCount);
            } else {
